# Reverse Benchmark — Run 1: Airbnb × Innovator's Dilemma (Part 1)
...
- **Book location**: `STACK_DIR/Agents-data-source/Books/Books/` (the filename contains a Unicode right single quote — use the Read tool or Glob to find it)
- **Chapters to focus on**: Part 1, Chapters 1-4 — "Why Great Firms Fail"
  - Ch 1: How Can Great Firms Fail? (disk drive industry as disruption microcosm)
  - Ch 2: Value Networks and the Concept of Disruption
  - Ch 3: Disruptive Change in the Mechanical Excavator Industry
  - Ch 4: What Goes Up, Can't Go Down (the one-way ratchet upmarket)
- **Book Guide**: `STACK_DIR/Agents-data-source/Books/Book Guide/Books_Summary.md` (entry #17)
- **Analysis angle**: Airbnb as the INCUMBENT being disrupted — they moved upmarket (luxury experiences, higher fees, professional hosts) and left the bottom of the market exposed. Split Lease is the disruptive entrant serving the overlooked segment (mid-term shared housing).
...
4. Write `STACK_DIR/runs/{RUN_ID}/run-config.json`:
   ```json
   {
     ...
     "lens": {
       "competitor": "airbnb",
       "book": "The Innovator's Dilemma (Clayton M. Christensen)2000.pdf",
       "book_chapters": "Part 1, Chapters 1-4: Why Great Firms Fail"
     },
     "status": "running",
...
Chapters: Part 1 (Ch 1-4) — Why Great Firms Fail
Analysis angle: Airbnb as the INCUMBENT being disrupted — they moved upmarket and left the bottom exposed.

## How to Read the Book
Read the full book PDF from disk. The file is at:
  STACK_DIR/Agents-data-source/Books/Books/

The filename contains a Unicode right single quote (U+2019). Use the Glob tool to find it:
  Glob pattern: *Innovator*Dilemma*

Focus on Part 1, Chapters 1-4:
- Chapter 1: How Can Great Firms Fail? — The disk drive industry shows how leaders fail at each architecture shift
- Chapter 2: Value Networks and the Concept of Disruption — Companies are trapped in value networks that define what "better" means
- Chapter 3: Disruptive Change in Mechanical Excavators — Incumbents saw the new tech, understood it, but couldn't pursue it
- Chapter 4: What Goes Up, Can't Go Down — Cost structures and margin expectations create a one-way ratchet upmarket
...
|-------|-----------|-------------|---------|------------|
| 0 | 0-competitor-mapping.md | competitor-journey.json | book PDF (Ch 1-4), book guide, journey map | — |
| 1 | 1-disappointment-mining.md | disappointments.json | book PDF (Ch 1-4), book guide | 0 |
| 2 | 2-transcript-crossref.md | crossref-insights.json | book PDF (Ch 1-4), element library, host+guest call transcripts | 0, 1 |
| 3 | 3-opportunity-identification.md | opportunities.json | book PDF (Ch 1-4), element library | 0, 1, 2 |
...
- Book PDF: `STACK_DIR/Agents-data-source/Books/Books/` — Glob for `*Innovator*Dilemma*`. Focus on Part 1, Ch 1-4.
...
Read these transcripts from disk. Search for mentions of "airbnb", competitor experiences, pricing frustrations, upmarket drift, and keywords related to the disappointments from Layer 1.
```
